...
export const addGuest = (guest) => async (dispatch) => {
  try {
    setLoading();
    const res = await axios.post('http://localhost:5000/guest', guest);
    dispatch({
      type: ADD_GUEST,
      payload: res.data,
    });
...
export const editGuest = (guest) => async (dispatch) => {
  try {
    setLoading();
    const res = await axios.put(`http://localhost:5000/guest/${guest.id}`, guest);
    dispatch({
      type: EDIT_GUEST,
      payload: res.data,
    });
...
export const searchGuest = (text) => async (dispatch) => {
  try {
    setLoading();
    const res = await axios.get(`http://localhost:5000/guest?q=${text}`);
    dispatch({
      type: SEARCH_GUEST,
      payload: res.data,
    });
